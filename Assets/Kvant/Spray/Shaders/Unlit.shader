...
Shader "Kvant/Spray/Transparent Unlit"
{
    ...
    {
        half4 c = (float4) 0.0 ; // i.color;
    #if _MAINTEX

        float2 spriteUV = i.texcoord;

        float2 dim = float2(16.0, 10.0);

        float2 grid = floor(i.texcoord1 * dim) / dim;

        grid.x =  nrand(i.texcoord1, 90.0);
        grid.y =  nrand(i.texcoord1, 25.0);

        grid = floor(grid * dim) / dim;
        
        spriteUV /= dim;
        spriteUV += grid;
        
        spriteUV = 1.0 - spriteUV;
