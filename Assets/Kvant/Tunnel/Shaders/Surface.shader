...
Shader "Kvant/Tunnel/Surface"
{
    ...
    SubShader
    {
        ...
        #if _ALBEDOMAP || _NORMALMAP || _OCCLUSIONMAP
            data.localCoord = v.texcoord.xyx + _BufferOffset.xyx;
        ...
        #if _ALBEDOMAP
            // Base color
            // o shit fuckin trilinear 
            
            float2 uv = frac(_MapScale.z * IN.localCoord.xy); 
            uv = (uv + _MapScale.xy) / 16;
          //  uv = uv * 0.8 + 0.1;
            half4 color = tex2Dlod(_MainTex, float4(uv, 0,0));
